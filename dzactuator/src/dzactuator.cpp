...
/**
 * @brief 速度指令订阅回调函数（处理cmd_vel话题）
 * @param msg 订阅的速度指令消息（包含线速度和角速度）
 * @details 根据订阅的线速度和角速度，计算并设置移动底盘的目标速度、目标角度及方向标志
 */
void turn_on_robot::callback_cmd_vel_angle(const geometry_msgs::Twist::ConstPtr &msg)
{
  // printf("callback_cmd_vel_angle\n");
  float v = msg->linear.x;   // 提取线速度（m/s）
  float w = msg->angular.z;  // 提取角速度（rad/s）

  // 计算目标速度（范围限制：0~255）
  // 公式：v * 23 / 0.33 为速度转换系数（可能与电机PWM值相关）
  // clamp确保速度在[-255, 255]范围内，abs取绝对值（方向由SpeedDirection单独控制）
  moveBaseControl.TargetSpeed = abs(clamp(v * 23 / 0.33, -255.0, 255.0));

  // 计算目标转向角度（仅当线速度非零时有效）
  if (!v == 0)  // 线速度非零（避免除零错误）
  {
    // 公式：atan(CARL * w / v) 计算转向角（弧度），其中CARL可能为机器人轴距或转向相关参数
    // 乘以57.3将弧度转换为度数（180/π≈57.3）
    moveBaseControl.TargetAngle = round(atan(CARL * w / v) * 57.3);
  }
  else  // 线速度为零（原地转向或停止）
  {
    moveBaseControl.TargetAngle = 0;  // 角度置零
  }

  // 角度偏移补偿（可能为硬件或机械结构的初始角度偏移）
  moveBaseControl.TargetAngle += 60;

  // 角度范围限制（确保在20~100之间）
  if (moveBaseControl.TargetAngle < 20)
  {
    moveBaseControl.TargetAngle = 20;  // 最小角度限制
  }
  if (moveBaseControl.TargetAngle > 100)
  {
    moveBaseControl.TargetAngle = 100;  // 最大角度限制
  }

  // 设置速度方向标志（控制电机正反转）
  if (msg->linear.x > 0)
    moveBaseControl.SpeedDirection = 0x00;  // 正向（线速度>0）
  else if (msg->linear.x < 0)
    moveBaseControl.SpeedDirection = 0x01;  // 反向（线速度<0）
  else if (msg->linear.x == 0)
    moveBaseControl.SpeedDirection = 0x00;  // 停止时默认正向（或保持上一状态）
  // // 角度方向标志（注释未启用）
  // if (moveBaseControl.TargetAngle > 0)
  //   moveBaseControl.TargetShiftPosition = 0x20; // 左转
  // else if (moveBaseControl.TargetAngle < 0)
  //   moveBaseControl.TargetShiftPosition = 0x10; // 右转
  // else if (moveBaseControl.TargetAngle == 0)
  //   moveBaseControl.TargetShiftPosition = 0x00; // 不转向
}
...
/**
 * @brief 云台电机速度PID控制器（位置式PID）
 * @param target_pose 目标位置（编码器值）
 * @param current_pose 当前位置（编码器值）
 * @return double 计算出的电机目标速度（范围：0~max_speed）
 */
double turn_on_robot::CaremaSpeedControl(int target_pose, int current_pose) {
    // 静态变量：用于跨调用保持PID状态（避免重复初始化）
    static double prev_error = 0.0;       // 前一次误差（用于微分项计算）
    static double integral = 0.0;         // 积分项（用于消除稳态误差）
    static double filtered_derivative = 0.0; // 滤波后的微分项（减少高频噪声）

    // PID控制参数（静态常量，全局固定）
    static constexpr double kp = 40.0;              // 比例增益：放大当前误差的影响
    static constexpr double ki = 0.01;             // 积分增益：消除稳态误差（当前未启用）
    static constexpr double kd = 0.03;             // 微分增益：抑制误差变化率（当前未启用）
    static constexpr double max_speed = 3700.0;    // 电机最大速度限制（防止过冲）
    static constexpr double sampling_time = 0.01;  // 采样时间（10Hz控制频率）

    // 1. 计算位置误差（当前核心控制量）
    double error = static_cast<double>(target_pose - current_pose); // 目标位置与当前位置的差值

    // 2. 积分项更新（带限幅防止积分饱和）
    integral += error * sampling_time;  // 积分项 = 误差 * 时间累积
    integral = clamp(integral,          // 限制积分项范围（避免长时间单向误差导致积分过大）
                     -max_speed / (2.0 * ki),  // 下限：防止负向积分饱和
                     max_speed / (2.0 * ki));  // 上限：防止正向积分饱和

    // 3. 微分项更新（带低通滤波）
    double derivative = (error - prev_error) / sampling_time;  // 微分项 = 误差变化率 / 时间
    filtered_derivative = 0.8 * filtered_derivative + 0.2 * derivative;  // 一阶低通滤波（保留80%历史值+20%当前值）

    // 4. PID控制计算（当前仅使用比例控制P）
    // 完整PID公式：speed = kp*error + ki*integral + kd*filtered_derivative
    double speed = kp * error;  // 仅使用比例项（可能为调试阶段临时配置）

    // 5. 速度后处理（确保输出有效）
    speed = std::abs(speed);               // 速度取绝对值（电机方向由其他逻辑控制）
    speed = clamp(speed, 0.0, max_speed);  // 限制速度不超过最大值

    // 6. 保存当前误差用于下次计算
    prev_error = error;

    return speed;  // 返回最终计算的电机目标速度
}
